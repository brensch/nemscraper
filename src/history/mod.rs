...
pub fn record_processed_csv_data<P: AsRef<Path>>(
    ...
    appender.flush()
        .with_context(|| {
            format!(
                "Failed to flush appender for history table while processing ZIP '{}'. This might be due to a primary key violation if records already exist.",
                zip_filename_str
            )
        })?;
...
pub fn get_distinct_processed_csv_identifiers(conn: &Connection) -> Result<Vec<String>> {
    ...
    let mut query_stmt = conn
        ...
        .with_context(|| {
            format!(
                "Failed to prepare statement for distinct table schema names from '{}'",
                HISTORY_TABLE_NAME
            )
        })?;

    let identifiers_iter = query_stmt
        .query_map([], |row| row.get(0))
        .with_context(|| {
            format!(
                "Failed to query distinct table schema names from '{}'",
                HISTORY_TABLE_NAME
            )
        })?;

    let mut identifiers = Vec::new();
    for identifier_result in identifiers_iter {
        identifiers.push(
            identifier_result
                .with_context(|| "Failed to retrieve a table schema name from query results")?,
        );
    }

    debug!(
        "Retrieved {} distinct processed CSV identifiers (table schema names).",
        identifiers.len()
    );
    Ok(identifiers)
}
